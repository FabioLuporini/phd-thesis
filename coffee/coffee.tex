...
The storage layout of $A$, however, is incorrect after the application of this outer-product-based vectorization (\emph{op-vect}, in the following). It can be efficiently restored with a sequence of vector shuffles following the pattern highlighted in Figure~\ref{fig:restore-layout}, executed once outside of the \texttt{ijk} loop nest. The pseudo-code for the Burgers equation when using \emph{op-vect} is shown in Listing~\ref{code:helmholtz-opvect}.
...
\KwSty{void} burgers(\KwSty{double} A[4][4], \KwSty{double} **coords) $\lbrace$\\
~~// K, det = Compute Jacobian (coords) \\
~~// Declaration of basis function matrices \\
~~\\
~~\KwSty{for} (\KwSty{int} i = 0; i$<$3; i++) $\lbrace$ \\
~~~~// Do expression rewriting \\
...
~~$\rbrace$\\
$\rbrace$
\caption{Local assembly code for the Burgers problem after vector-register tiling (outer-product vectorization), expression rewriting, padding and data alignment. In this example, the unroll-and-jam factor is 1.}
...
through additional branch instructions. Finally, it might affect register locality: for instance, the same array could be accessed in different sub-expressions, requiring a proportional number of loads be performed. This is not the case for the Helmholtz example. Nevertheless, as shown in Section~\ref{sec:perf-results}, the performance gain from improved register reuse along inner dimensions can still be greater, especially if the split factor and the splitting itself use heuristics to minimize the aforementioned issues.


\subsection{Exposing Linear Algebra Operations}
