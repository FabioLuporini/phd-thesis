...
\KwSty{void} weighted$\_$laplace(\KwSty{double} A[3][3], \KwSty{double} **coords, \KwSty{double} **w) \\
$\lbrace$ \\
~~  // Compute Jacobian \\
~~  \KwSty{double} J[4]; \\
...
~~\\
~~  // Basis functions \\
~~  \KwSty{static const double} A[1][3] = $\lbrace\lbrace$ -0.999999999999999, ...$\rbrace\rbrace$ ;\\
~~  \KwSty{static const double} B[1][3] = $\lbrace\lbrace$ -1.0, ...$\rbrace\rbrace$ ;\\
~~  \KwSty{static const double} C[1][3] = $\lbrace\lbrace$ 0.333333333333333, ...$\rbrace\rbrace$ ;\\
~~\\
...
\subsubsection{More Examples and Motivations for Optimizations}
...
\item data locality
\end{itemize}
Three conceptually distinct classes of transformations can be individuated, which we refer to as \textit{expression rewriting}, \textit{code specialization}, and \textit{general-purpose optimizations}. 

Expression rewriting mainly targets arithmetic intensity by transforming the assembly expression so as to minimize the number of floating point operations required to evaluate the local element matrix. Code specialization is tailored to optimizing for instruction-level parallelism, particularly SIMD vectorization, and data (register) locality. Both classes of transformations are inspired by the inherent structure of local assembly code and make use of domain knowledge, which is the intuitive reason for which standard general-purpose compilers cannot be as much effective. The third class of transformations concerns general-purpose optimizations; that is, generic, well-known techniques for improving code performance, such as loop unrolling or loop interchange.
...
\subsubsection{Results of General Applicability}
...
\subsubsection{Performance of Helmholtz}
\begin{figure}[t]
...
\subsubsection{Performance of Elasticity}
\begin{figure}[t]
...
\subsubsection{Performance of Poisson}
\begin{figure}[t]
...
\subsubsection{Performance of Hyperelasticity}
\begin{figure}[t]
...
\subsubsection{Details on the Autotuning Process}
\label{sec:coffee-pereval-autotune}

\paragraph{Autotuning Overhead}
...
\paragraph{Selected Optimizations}
