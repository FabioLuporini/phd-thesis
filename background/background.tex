...
The conception and the design of the COFFEE layer is one of the main contributions of this thesis. Chapters~\ref{ch:optimality},~\ref{ch:lowlevelopt} and~\ref{ch:coffee} are entirely devoted to this topic.
...
An interesting approach is adopted in Pochoir~\citep{pochoir}, in which a compiler translates the high-level specification into cache-oblivious algorithms for multi-core CPUs. Interestingly, an attempt at integrating this system with a higher-level domain specific language for finite difference computations failed due to constraints on the programming interface~\citep{tj-thesis}. 
...
Reordering transformations can be composed to further improve their effectiveness. A case of particular interest for this thesis is the composition of loop fusion and loop tiling: first, a sequence of loops is fused into a single loop; then, iterations are grouped together into tiles. Such a transformation can turn data reuse across consecutive loops into data locality, thus optimizing memory bandwidth and memory latency. Composing loop fusion and loop tiling is however cumbersome, in general; automation is even more challenging, especially depending on the kind of stencils that need be supported.

In the context of computational methods for approximating the solution of PDEs, the composition of loop fusion and loop tiling is often referred to as {\em time tiling}. In these codes, a sequence of loops over the spatial dimensions is executed within a time loop; time tiling fuses the spatial loops and builds tiles spanning the time dimension. Time tiling has in particular been studied for structured stencil codes~\citep{cohen-timetiling,ics-stencil-tiling,Zhou12,pluto}. Time tiling for unstructured stencil codes -- also known as {\em sparse tiling} -- has instead received limited attention, because of the complexity inherent in performing data dependency analysis. Prior to this thesis, sparse tiling had only been applied ``manually'' to individual benchmarks, through ``ad-hoc'' strategies. As detailed in Chapter~\ref{ch:sparsetiling}, we advance the state-of-the-art by introducing:
\begin{itemize}
\item A technique for applying sparse tiling to {\it arbitrary} sequences of loops expressed through the {\em loop chain abstraction}~\cite{ST-KriegerHIPS2013}. The loops, in particular, may include unstructured stencils. This contribution was a joint effort with~\cite{st-paper}.
\item A system that automates this technique and enables execution on distributed-memory architectures.
\end{itemize}

Independently of the nature of the stencil, there exist two basic approaches to time tiling, which differ in the way tiles are computed:
\begin{description}
\item[Split tiling] Differently shaped tiles are computed. The tile shape varies across the sequence of fused loops. Tiles must be scheduled such that all tile-to-tile dependencies are satisfied. In the context of structured stencil codes, several split tiling schemes have been studied. These schemes usually take the name from the geometry of the tile shape they end up producing, such as {\em hexagonal tiling} or {\em diamond tiling}. Split tiling schemes differ in the achieved trade-off between parallelism and data locality.
\item[Overlapped tiling] Tiles can share subsets of iterations. A shared iteration is ``owned'' by only one tile and is executed redundantly by the set of ``overlapping'' tiles. These tiles store intermediate values into ``ghost'' regions of memory. This approach allows all tiles to execute in parallel without the need for synchronization, at the price of redundant computation.
\end{description}
Some of these tiling schemes have been reviewed in~\cite{survey-tile-shape}. In this thesis, we will employ a mixed split-overlapped sparse tiling scheme for unstructured stencil codes suitable for distributed-memory architectures.

% sparse tiling is a split scheme

\subsection{Automation via Static Analysis}
\label{sec:bkg:poly}
There are two alternative approaches to the automation of loop reordering transformations. Both of them rely on static analysis of source code:

\begin{description}
\item[Graph-based Representation] General-purpose compilers analyze the source code and produce an intermediate representation (IR) usually based on a graph-like data structure (e.g., SSA in LLVM). Special algorithms assert the legality of a transformation, while cost models are used to predict the impact on performance. Sometimes the cost models are exposed to the user by suitable reports (e.g., detailed vectorization reports can be requested to the Intel compiler). Users have some form of control over the optimization process. Pragma directives can be used to choose explicitly how to optimize a loop nest (e.g., to set a specific unroll factor or to enforce vectorization when the compiler erroneously thinks it is unprofitable), while compiler parameters allow tuning of optimization heuristics. 
\item[Polyhedral Model] Several research compilers, and more recently a fork of the LLVM compiler itself through a module called Polly~\cite{polly}, apply reordering transformations working on geometric representations of loop nests. To model a loop nest as a {\em polyhedron}, two conditions must be satisfied: (i) loop bounds as well as array indices must be expressions affine in the loop indices and (ii) pointer aliasing must be known at compile-time. These conditions are often necessary in the case of graph-based IRs too, although in a more relaxed fashion (e.g., the Intel compiler can vectorize, to some extent, non-affine loop nests). Polyhedral compilers target parallelism and data locality by composing {\em scheduling functions}. A schedule defines the order in which the iterations of a loop nest are executed; a scheduling function can be applied to change the original order. Once the polyhedron is available, different scheduling functions can be compared and applied.
\end{description}

A compiler needs a model of the data dependence pattern to apply a loop reordering transformation. If this model can be built, the legality of a transformation can be determined. In case a transformation is found legal, a cost model is used to predict its impact on the performance. If also the cost model provides positive feedback, the transformation is finally introduced by manipulating the IR. 
...
In this thesis, we focus on irregular codes, or unstructured stencil codes, as those arising in finite element and finite volume methods. Unstructured stencils render loop nests non-affine, thus precluding the adoption of polyhedral compilation. Recently, there has been an effort in extending the polyhedral model to non-affine loops~\cite{sparse-poly-framework}, but we hardly believe this model will ever be applicable in the real-world applications we are interested in.
