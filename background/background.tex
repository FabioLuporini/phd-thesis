...
\item[Stencils for unstructured meshes] Given an element $\boldsymbol{i}$ in the mesh and an access function $g$, a stencil is a vector-valued operator $f(\boldsymbol{i}, g) = [f_1(\boldsymbol{i}, g), f_2(\boldsymbol{i}, g), ..., f_n(\boldsymbol{i}, g)]$ which retrieves the $n$ elements that need to be accessed when updating $\boldsymbol{i}$. There usually is no closed form for a function $f_j$.
\end{description}
