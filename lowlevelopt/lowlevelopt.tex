...
\item[Vector-register tiling] Tiling at the level of vector registers exploits the peculiar memory access pattern of finite element operators (i.e., inner products involving test and trial functions) to augment data locality.
...
\end{description}
We will also provide insights into the effects of more ``traditional'' compiler optimizations: loop unroll, loop interchange, loop fusion and vector promotion. Most of these will have to be enforced at code generation time, to overcome the limitations of general-purpose compilers.
...
~~\KwSty{double} $\_$A[4][4] ALIGN = $\lbrace\lbrace$0.0$\rbrace\rbrace$;\\
~~\\
...
~~$\rbrace$\\
~~\KwSty{for} (\KwSty{int} i = 0; i$<$6; i++) $\lbrace$ \\
~~~~\KwSty{double} T$\_$0[3] ALIGN;\\
~~~~\KwSty{double} T$\_$1[3] ALIGN;\\
