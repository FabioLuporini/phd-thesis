...
\end{Strategy}
In the current example, we observe that, depending on the size of $L_i$, applying Strategy~\ref{strategy:ii} could reduce the operation count since the expression would be recast as $v^0_i v^0_i a a + v^0_i v^1_i (ab + ab) + v^1_i v^1_i c c + ...$ and some hoistable sub-expressions would be exposed. On the other hand, Strategy~\ref{strategy:i} would have no effect as $v$ only depends on a single loop, $L_i$. A second application of Strategy~\ref{strategy:ii} was provided in Figure~\ref{code:multi_loopnest}. In general, the choice between the two strategies depends on multiple factors: the loop sizes, the increase in operation count due to expansion (in Strategy~\ref{strategy:ii}), the gain due to code motion, and the presence of common sub-expressions. These examples motivate the introduction of a particular class of expressions, for which the two strategies assume notable importance.
...
It is finally straightforward to determine in which range of levels (if any) the application of Strategy~\ref{strategy:ii} improved the operation count. The cost and the effectiveness of the {\em Strategy Selection Analysis} obviously depend on the number $n$ of pairs ${<}i_1, i_2{>}$ that are evaluated (at most $k^2$). Since during our experiments setting $n = k^2$ never led to unacceptable code generation times\footnote{Even in the case of complex hyperelastic models, the code generation time never exceed a few seconds on a relatively old Sandy Bridge architecture.}, in the following we assume that the {\em Strategy Selection Analysis} always considers all possible pairs.


\subsection{The Sharing Elimination Algorithm}
\label{sec:se-algo}
Algorithm~\ref{algo:sharing-elimination} gathers the considerations from the previous sections and describes sharing elimination in its entirety. Examples will be provided in Section~\ref{sec:se-examples}.
...
In Figure~\ref{code:poisson}, Algorithm~\ref{algo:sharing-elimination} is executed in a simple yet realistic scenario, which originates from the bilinear form of a Poisson equation in two dimensions. The temporaries graph, which consists of a single level, is used by the {\em Strategy Selection Analysis}. This leads to the synthesis in Figure~\ref{code:poisson_b}. The sharing graph is $G = (\lbrace t_0, t_1, t_2, t_3 \rbrace, \lbrace (t_0, t_2), (t_1, t_3) \rbrace)$, but since there are no factorization opportunities the ILP formulation has no impact.
