...
\begin{description}
\item[quad] Optimized quadrature mode. Work presented in~\cite{quadrature-olegaard}, implemented in the FEniCS Form Compiler.
...
\begin{figure}
 \makebox[\textwidth][c]{\includegraphics[scale=0.77]{optimality/perf-results/elasticity}}
...
\item[$\mathrm{nf}=2, 3$] \texttt{auto} reverts to \texttt{quad}, which would theoretically be the right choice (the flop count is much lower than in \texttt{tens}); however, the generated code suffers from the presence of indirection arrays, which break autovectorization and ``traditional'' code motion.
\end{description}
