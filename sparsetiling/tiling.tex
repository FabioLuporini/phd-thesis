...
\section{sec:tiling:lc-unstruct}
...
Dependencies between non-adjacent loops are also possible. Our algorithm takes care of them by inspecting \textit{projections}. In order to schedule $L$ to tiles, the ``most recent'' projection for each space accessed by $L$ must be available. For example, assume that the space $S$ is read by $L_k$ and is written by both $L_i$ and $L_j$, with the loop chain being such that $L_i \prec ... \prec L_j \prec ... \prec L_k$. Let us denote by $\rho_S$ the projection for $S$. Each time that a loop accessing $S$ in write mode is inspected for scheduling, $\rho_S$ must be updated. When eventually scheduling $L_k$, therefore, an up-to-date snapshot of $\rho_S$ (i.e., the one produced right after tiling $L_j$) will be used to compute a legal tiling.
...
A major constraint of the inspector is that only the first loop of the chain, $L_0$, can be chosen as seed loop, or $L_{seed}$. Any loop in the chain could theoretically be picked as seed, but this would make supporting distributed memory parallelism cumbersome because of the non-trivial coordination that would be required between core and boundary tiles. If only shared memory parallelism is requested, the seed loop can be chosen arbitrarily. If we have $L_0 \prec ... \prec L_{seed}$, then we need a mechanism for propagating tiling ``backwards'', that is, from $L_{seed}$ towards $L_0$. In~\cite{st-paper}, we propose two ``symmetric'' algorithms to solve this problem, \textit{forward tiling} and \textit{backward tiling}, where the latter simply uses the $MIN$ function in place of $MAX$. In the algorithm we present here, the fact that $L_{seed} = L_0$ means that tiles only grow ``in one direction'' (i.e., only projections for $[L_0,\ L_1,\ ...,\ L_{i-1}]$ may be required), so we do not need this distinction\footnote{The algorithm that is implemented in the SLOPE library, however, is more general and supports backwards tiling as well.}. 

\begin{Algo}[Sparse Tiling Inspector]
\label{algo:st-inspector}
\normalfont 
The input are a loop chain $\mathbb{L}$, as described in Section~\ref{sec:tiling:lc-unstruct}, an average tile size $ts$, and an execution mode $mode$.
\begin{enumerate}
\item Pick $L_{seed}$. If $mode$ indicates a pure shared memory execution, then we heuristically select the loop in the middle of the loop chain. otherwise $L_{seed} = L_0$. Let $S_{seed}$ be the iteration space of $L_{seed}$.\\
...
\item Partition the core region of $L_{seed}$ into $\lbrace P_0, P_1, ..., P_{m-1}\rbrace$ such that $P_i \cap P_j = \emptyset$ and $\cup_{i = 0}^{m-1} P_i = S_{seed}$. The partitioning is sequential: $S_{seed}$ is ``chunked'' every $ts$ iterations, with $P_{m-1}$ that may be smaller. Similarly, partition the boundary region of $L_{seed}$ into $k$ subsets.

\item Create $m+k+1$ tiles, one for each partition, plus one extra tile for the non-exec region of $L_{seed}$; $\mathbb{T} = [T_0, ..., T_{m+k}]$. Each tile $T_i$ has four fields: a region (where possible values are core, boundary and non-exec); a color; for each loop $L$, a list of iterations $T_i^L$ (initially empty, to be populated by the inspection algorithm); for each loop, a list of ``local maps'', whose role will become clear next. This structuring is illustrated in Figure~\ref{fig:tile-structure}. $T_i$ is forthwith initialized with the iterations in $P_i$; that is, $T_i^{L_{seed}} = P_i$.

\item Assign colors to tiles. If $mode$ requests shared memory parallelism, color the tiles such that adjacent tiles are not given the same color. Otherwise, assign increasing colors to tiles (i.e., $T_i$ given color $i$). The boundary tiles are always given colors higher than that of core tiles; the non-exec tile has highest color. In essence, this is to provide the executor with sufficient information to schedule all core tiles first, then all boundary ones, and finally the non-exec tile.\\
\textit{The adjacency relation is determined through the maps available through the descriptors of the loop chain.}

\item Initialize $\rho$, a data structure that keeps projections for each space in the loop chain. Then, for each loop $L_i$ in $[L_1, L_2, ..., L_{m-1}]$:
\begin{itemize}
\item Update $\rho$ using the descriptors in $L_{i-1}$, as explained in Algorithm~\ref{algo:st-projection}. Along with $\rho$, the projection algorithm returns a piece of information indicating whether two different tiles having the same color will become adjacent after tiling. Let $C \in \mathbb{N}^{m \times m}$ be a matrix tracking conflicting colors, with $C[i,j] = 1$ indicating that $T_i$ and $T_j$ have the same color and will eventually become adjacent.
\item Use $\rho$ to schedule the iteration of $L_i$ to tiles. The tiling algorithm is given in Algorithm~\ref{algo:st-tiling} 
\end{itemize}

\item If $C$ indicates the presence of at least one conflict, say between $T_i$ and $T_j$, add a ``fake'' connection between $T_i$ and $T_j$ and loop back at step (4); the coloring phase now will interpret $T_i$ and $T_j$ as being connected, thus assigning different colors. Otherwise, a legal tiling has been computed and the inspection phase can be terminated. 
