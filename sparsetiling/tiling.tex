...
The power of automation becomes evident in this section. The flexibility of the {\em loop$\_$chain} interface, the versatility of SLOPE allowing to apply sparse tiling to arbitrary sequences of loops, and the $S{-}depth$ mechanism make it possible to experiment with a variety of fusion schemes without touching the source code. 

We develop in total 6 fusion schemes, based on several criteria: (i) amount of data reuse in the loop chain(s), (ii) extent of the boundary region/amount of redundant computation, (iii) memory footprint of the loop chain(s) -- the larger, the smaller the tile size to fit in cache. The fusion schemes are summarized in Table~\ref{table:seigen-fusion-schemes}, while their specification is available at~\cite{seigen-code}.

\begin{table}[htpb]
\centering
\makebox[\textwidth][c]
{
\begin{tabulary}{1.0\columnwidth}{M{1.2cm} | M{2.0cm} | M{7.2cm} | M{1.7cm}}
\hline
Fusion scheme & $\#$(sub-)loop chains & Criterion & $S{-}depth$  \\
\hline \hline
{\tt fs1} & 3 & Fuse {\tt velocity} cells and facets loops & 2 \\ \hline
{\tt fs2} & 8 & {\tt fs1} and fuse {\tt stress} cells and facets loops & 2 \\ \hline
{\tt fs3} & 6 & {\tt fs2} and fuse some solver loops & 2 \\ \hline
{\tt fs4} & 3 & More aggressive than {\tt fs2} & 3 \\ \hline
{\tt fs5} & 2 & Maximize locality in the {\tt velocity} and {\tt stress} loop chains & 4 \\ \hline
{\tt fs6} & 1 & The extreme case: fuse all of the 25 loops & 8 \\
\hline
\end{tabulary}
}
...
Furthermore, several optimizations were tried. These optimizations are applicable to all of the fusion schemes.

\begin{description}
\item[Global and local maps] Algorithm~\ref{algo:st-inspector} computes so called local maps to avoid double indirections in the executor. We regard this as a potential optimization. Although there is no data reuse for the local maps, the benefits from hardware prefetching and memory latency could be dominant. We will experiment with both global (i.e., original, as provided by the application itself) and local (i.e., computed by SLOPE) maps.

\item[Tile shape] 
\item[Software prefetching]
\item[Extended boundary region]
\end{description}
