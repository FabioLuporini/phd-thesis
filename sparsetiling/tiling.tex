...
\item Block-diagonal, as a consequence of the employed spatial discretization. The inverse of a block-diagonal matrix is again a block-diagonal matrix, so it can straightforwardly be determined by computing the inverse of each block. Each block in an inverse matrix belongs to an element in the mesh. The solution of the linear system $Ax = b$, expressible as $x = b A^{-1}$, can therefore be evaluated by looping over the mesh and computing a ``small'' matrix-vector product in each element, where the matrix is a block in $A^{-1}$.
\end{itemize}
Assembling the global vectors reduces to a sequence of loops over mesh elements, particularly over cells, interior facets, and exterior facets. Overall, 25 loops are executed in each time loop iteration.
...
\begin{itemize}
\item Consecutive pairs of loops are characterized by indirect data dependencies. Therefore, not only does data reuse arise within a given loop (e.g., by accessing vertex coordinates from adjacent cells), but also across the entire loop chain. This makes Seigen a natural fit for sparse tiling. 
\item 8 loops perform matrix-vector multiplications in each mesh element. It is well established that this kind of linear algebra operations are memory-bound. 4 of these loops arise from {\tt velocity}, the others from {\tt stress}. There is significant data reuse amongst the 4 {\tt velocity} loops and amongst the 4 {\tt stress} loops, since the same blocks in the global inverse matrices are accesses. It is therefore reasonable to hypothesize performance gains if these loops were fused through sparse tiling.
...
\item 8 loops implement cell integrals, which contribute to the assembly of the 8 global vectors computed in the loop chain. 3 of these loops require evaluating a field, which we informally call {\tt absorption}. In {\tt explosive$\_$source}, and similarly in other test cases, {\tt absorption} is represented with a 4th order polynomial in a discontinuous-Galerkin function space. When $q$ is low, the operation count for evaluating {\tt absorption} is dominant: the 3 {\tt absorption} loops tend to be compute-bound and to outweigh not only the other 5 cells loops, but but also many of the interior facet loops. In the following, when discussing performance results, we will refer to this phenomenon as the ``absorption-effect''. With high values of $q$, the absorption-effect becomes less relevant. In theory, the absorption-effect could be eliminated entirely by pre-computing the field outside of the time loop, as  {\tt absorption} does not change over time. Unfortunately, this is not possible in the current version of Firedrake. 
...
\item The global matrix assembly cost -- an operation that takes place before entering the time loop -- {\it is not} included in the execution time. The cost is not affected by sparse tiling in any way. The reason is that, in Seigen, the diagonal of a global matrix needs be copied into a vector, and this copy at the moment occurs in in Python. The correct approach would be having Firedrake assembling block-diagonal matrices directly into vectors, at the C layer. Then, global matrix assembly would have a negligible cost, which motivates our timing choice. This is however work in progress.
\item The cost of the sparse tiling inspection {\it is} included in the execution time.
\item Additional costs were minimized: no check-pointing, only two I/O sessions (at the beginning and the end of the computation), and minimal logging.
\item The time loop has a fixed duration $T$, while the time step varies with the element spacing. In essence, for a given wave velocity, the finer the mesh, the smaller the time step needs be to ensure that the simulation converges. Most importantly, $T$ cannot be decreased. A significant amount of time is necessary for the waves to fully propagate across the domain; before that happens, the value of most fields remains unchanged (typically, $0.0$) all over the domain. This appears to reduce the data movement, with the execution of elements crossed by the waves taking (much) longer than that of all other elements. Summarizing, all execution times derive from real, complete seismological simulations.
