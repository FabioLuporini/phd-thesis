...
\makebox[\textwidth][c]
{
\begin{tabulary}{1.2\textwidth}{M{2.5cm} | M{3.5cm} | M{3.5cm} | M{3.5cm} N}
\hline
...
The flexibility of the {\em loop$\_$chain} interface, the versatility of SLOPE allowing to apply sparse tiling to arbitrary sequences of loops, and the $S{-}depth$ mechanism make it possible to experiment with a variety of fusion schemes without touching the source code. 

We devise 6 fusion schemes, based on several criteria: (i) amount of data reuse in the loop chain(s), (ii) extent of the boundary region/amount of redundant computation, (iii) memory footprint of the loop chain(s) -- the larger, the smaller the tile size to fit in cache. The fusion schemes are summarized in Table~\ref{table:seigen-fusion-schemes}, while their specification is available at~\cite{seigen-code}.
...
\hline
Fusion scheme & $\#$(sub-)loop chains & Criterion & $S{-}depth$  \\
\hline \hline
...
\caption{Summary of the fusion schemes adopted in Seigen. The $S{-}depth$ column is to be interpreted as number of strips of off-process cells.}
...
Sparse tiling, and more generally time tiling, aim to turn the data reuse in a sequence of loops into data locality. In this chapter, three problems have been tackled: the generalization of sparse tiling to arbitrary sequences of irregular loops through the loop chain abstraction; automation via DSLs; effective support for shared- and distributed-memory parallelism. The major strength of this work is that all algorithmic and technological contributions derive from an in-depth study of real-world applications. The performance issues arose in Seigen would have never been exposed if a set of more or less simplistic benchmarks had instead been used. We hope that our research could be in this sense inspirational: too often ``toy programs'' or proxy applications are used as a replacement of realistic computations, and this can be dramatically misleading.

Extensive experimentation is the most impelling need of this research. Although the thorough performance analysis of Seigen shed many lights on sparse tiling, testing other applications (not necessarily using finite elements) is fundamental to characterize and stretch the limits of the technique. Unfortunately, this type of analyses require a great deal of effort. Yet another open question of this chapter concerns the mesh numbering. The {\tt chunk} and {\tt metis} partitioning strategies were tested, but, as explained, our hypothesis is that much higher gains may be obtained by using Hilbert curves. Finally, an essential requirement of this work is the definition of a cost model to automatically derive fusion schemes or to drive auto-tuning systems. 
