...
\begin{itemize}
\item $\mathbb{L}$ consists of $n$ loops, $L_0, L_1, ..., L_{n-1}$. There are no global synchronization points between the loops.
...
\item The execution order of a loop's iterations does not influence the result. This means that given $L_i$, we can choose an arbitrary scheduling of iterations because there will not be loop-carried dependencies. 
...
\item A set $S$ is logically split into three contiguous regions: core ($S^{core}$), boundary ($S^{bound}$), and non-exec ($S^{nexec}$). This separation is essential for distributed memory parallelism. Given a process $P$ and a set $S$, we have that:
\begin{description}
 \item[$S^{core}$] The iterations of $S$ that exclusively belong to $P$.
 \item[$S^{bound}$] The boundary region can be seen as the union of two sub-regions, $S^{owned}$ and $S^{exec}$. $S^{owned}$ are iterations that belong to $P$ which will be redundantly executed by some other processes; $S^{exec}$ are iterations from other processes which are redundantly executed by $P$. We will see that redundant execution is exploited to preserve atomic execution; that is, the fact that all tiles can execute their iterations without the need for synchronization.
  \item[$S^{nexec}$] These are iterations of other processes that are communicated to $P$ because they need be indirectly read to correctly compute $S^{bound}$.
 \end{description} 
...
With respect to the original definition, one of the most important difference is the lack of data spaces. In unstructured mesh applications, a loop tends to access a large number of data spaces associated with the same set, so the idea is to perform data dependency analysis using sets and maps, rather data sets and access relations. This can significantly improve the inspection cost, because typically $|\mathbb{S}| << |\mathbb{D}|$. The second fundamental difference is the characterization of sets into the three regions core, boundary and non-exec. 
...
The seed loop $L_{seed}$ is used to initialize the tiles. Theoretically, any loop in the chain can be chosen as seed. Supporting distributed memory parallelism, however, is cumbersome if $L_{seed} \neq L_0$. This is because more general partitioning and coloring schemes would be needed to ensure that no iterations in $S_{seed}^{bound}$, the boundary region of $S_{seed}$, are eventually assigned to any of the core tiles. A constraint of our inspector algorithm in the case of distributed memory parallelism, therefore, is that $L_{seed} = L_0$. 

In the special case in which only shared memory parallelism is requested, where there is no need to distinguish between core and boundary tiles, the seed loop can be chosen arbitrarily. If we however choose $L_{seed}$ such that $L_0 \prec ... \prec L_{seed} \prec ...$, then a mechanism for constructing tiles in the reverse direction (``backwards''), from $L_{seed}$ towards $L_0$, is necessary. In~\cite{st-paper}, we propose two ``symmetric'' algorithms to solve this problem, \textit{forward tiling} and \textit{backward tiling}, with the latter using the $MIN$ function in place of $MAX$ when computing projections. For ease of exposition, and since in the fundamental case of distributed memory parallelism we impose $L_{seed} = L_0$, we here neglect this distinction\footnote{The algorithm that is implemented in the SLOPE library, however, is more general and supports backwards tiling for the case in which distributed memory parallelism is not required.}. 
...
\paragraph{Initialization of tiles}
...
\ForEach{$D$ in $L_i$.descriptors}{
  ...
  \eIf{$D$.map $== \perp$}{
    ...
    $S_{in}$, $S_{out}$, values, offsets $\gets$ inverse$\_$map\;
    $\phi_{S_{in}} \gets \perp$\; 
    \For{$e=0$ \KwTo $S_{in}$.size}{
       $\phi_{S_{in}}[e] \gets$ -1\;
      \For{$j=$ offsets[i] \KwTo offsets[i+1]}{
        adjacent$\_$tile = $\mathbb{T}$[values[j]]\;
        max$\_$color $\gets$ MAX($\phi_{S_{in}}[e]$.color, adjacent$\_$tile.color)\;
        \If{max$\_$color $\neq$ $\phi_{S_{in}}[e]$.color}{
          $\phi_{S_{in}}[e] \gets$ adjacent$\_$tile\;
        }
      }
    }
    update($C$, $\mathbb{T}$, $\phi_{S_{in}}$)\;
    $\Phi = \Phi \cup \phi_{S_{in}}$\;
  }
}
...
A projection tells which tile a set element belongs to at a given time instant of the tiling process. A new projection $\phi_{S_{in}}$ is needed if the elements of the set $S_{in}$ are written by $L_i$. Let us consider the challenging case, when writes or increments occur indirectly through a map. To compute $\phi_{S_{in}}$, we first determine the inverse map, as shown in Figure~\ref{fig:st-inverse-map}. Then, we iterate over all elements of $S_{out}$ -- the iteration space of $L_i$ -- that indirectly access elements in $S_{in}$. In particular, given $e \in S_{in}$, we determine the last tile (i.e., the tile with highest color) that writes to $e$, say $T_{last}$. We finally set $\phi_{S_{in}}(e) = T_{last}$. 
...
Using $\Phi$, we compute $\sigma_i$ as described in Algorithm~\ref{algo:st-tiling}. Finally, $\sigma_i$ is inverted and the iterations of $L_i$ are scheduled to the tiles. 
...
\ForEach{$D$ in $L_i$.descriptors}{
  \eIf{$D$.map $== \perp$}{
    ...
    $\phi_{S} \gets \Phi[D$.map.$S_{out}$]\;
    \For{$e=0$ \KwTo $S_i$.size}{
       $\sigma_i[e] \gets T_{\perp}$ \;
      \For{$j=0$ \KwTo arity}{
        adjacent$\_$tile $\gets \phi_{S}$[$D$.map.values[e*arity + j]]\;
...
\nonl ~\\
\ForEach{available color $c$}{
  \ForEach{$T \in \mathbb{T}^{bound}$ s.t. $T$.color $== c$}{
    execute$\_$tile($T$)\;
  }
}
