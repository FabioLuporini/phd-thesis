...
Previous work introduces an abstraction called the \emph{loop chain}\cite{KriegerHIPS2013}
...
written using the OP2~\cite{op2-main} library, where it is possible to derive a loop chain abstraction.
...
%One such pattern is the  \emph{loop chain}\cite{KriegerHIPS2013}.
...
%execution schedule across the loops~\cite{StroutLCPC2002}.
...
of jet engines, and in Volna~\cite{volna}, a CFD application for the 
...
\centerline{
        \begin{subfigure}
                \centering
                \includegraphics[scale=0.62]{sparsetiling/figures/op2-new-res/sb/time-intel-sandyb.pdf}\label{fig:airfoil-t-sb}
                \includegraphics[scale=0.62]{sparsetiling/figures/op2-new-res/west/time-intel-westmere.pdf}\label{fig:airfoil-t-w}
        \end{subfigure}%
}
\centerline{
	\begin{subfigure}
                \centering
                \includegraphics[scale=0.62]{sparsetiling/figures/op2-new-res/sb/speedup-intel-sandyb.pdf}\label{fig:airfoil-s-sb}
                \includegraphics[scale=0.62]{sparsetiling/figures/op2-new-res/west/speedup-intel-westmere.pdf}\label{fig:airfoil-s-w}
        \end{subfigure}%
}
...
The OP2 adaptation of the generalized sparse tiling technique was evaluated in a representative unstructured mesh application called Airfoil~\cite{AIRFOIL}. Three implementations of Airfoil, namely $omp$, $mpi$ and $tiled$, were compared on two shared-memory machines, an Intel Westmere (dual-socket 6-core Intel Xeon X5650 2.66 GHz, 12MB of shared L3 cache per socket) and a more recent Intel Sandy Bridge (dual-socket 8-core Intel Xeon E5-2680 2.00Ghz, 20MB of shared L3 cache per socket). The code was compiled using the Intel \texttt{icc 2013} compiler with optimizations enabled (\texttt{-O3}, \texttt{-xSSE4.2/-xAVX}).
...
%shown~\cite{StroutIJHPCA,StroutPLDI03,commAvoidingSparse2009}.
...
For unstructured codes, there has been various
...
% FIXME: might want to smooth this. 
%and were detailed in Section~\ref{sec:history}.
