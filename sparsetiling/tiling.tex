...
The second benchmark was Airfoil~\citep{AIRFOIL}, a proxy finite volume application designed in the context of the OP2 project to represent the 3D viscous flow calculations typical of the Hydra CFD code~\citep{hydra-op2} at Rolls-Royce plc. It is a nonlinear 2D inviscid airfoil code using an unstructured mesh, which solves the 2D Euler equations employing a scalar numerical dissipation. Airfoil has a predictor/corrector time-marching loop consisting of 2000 iterations, and iterates towards the steady state solution by using a control volume approach in each iteration~\citep{airfoil-code}. At each time iteration, four parallel loops are executed. The first two loops, called {\tt adt-calc} and {\tt res-calc}, constitute the bulk of the computation. The {\tt adt-calc} loop iterates over cells, reads from adjacent vertices and writes to a local dataset, whereas {\tt res-calc} iterates over edges and exploits indirect mappings to vertices and cells for incrementing indirect datasets associated to cells. These loops share datasets associated with cells and vertices. The {\tt res-calc} loop computes the flux over the interior edges. The third and fourth loops, called {\tt bres-calc} and {\tt update}, are less computationally expensive. The former computes the flux over the boundary edges, while the latter -- a direct loop over cells -- updates the current solution. Double-precision floating-point arithmetic is used.

Three implementations of Airfoil, {\em omp}, {\em mpi} and {\em tiled}, were compared on two shared-memory machines, an Intel Westmere (dual-socket 6-core Intel Xeon X5650 2.66 GHz, 12MB of shared L3 cache per socket) and an Intel Sandy Bridge (dual-socket 8-core Intel Xeon E5-2680 2.00Ghz, 20MB of shared L3 cache per socket). The code was compiled using the Intel \texttt{icc 2013} compiler with optimizations enabled (\texttt{-O3}, \texttt{-xSSE4.2} on the Westmere and \texttt{-xAVX} on the Sandy Bridge).
...
\includegraphics[width=\textwidth]{sparsetiling/figures/expl-source01.png}
\caption{Right after the explosion.}
\end{subfigure}%
~
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{sparsetiling/figures/expl-source02.png}
\caption{Wave propagation (1).}
\end{subfigure}%
~\\
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{sparsetiling/figures/expl-source03.png}
\caption{Wave propagation (2).}
\end{subfigure}%
~
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{sparsetiling/figures/expl-source04.png}
\caption{Final snapshot.}
\end{subfigure}%

\caption{Four phases of the {\tt explosive$\_$source} test case in Seigen, following an explosion at a point source.}
\label{fig:seigen-output}
\end{figure}

Seigen has several mechanisms to validate the correctness of the seismological model and the test cases. The numerical results of all code versions (with and without tiling) were checked and compared. Paraview was also used to verify the simulation output. A snapshot of the simulation output is displayed in Figure~\ref{fig:seigen-output}.
...
\item[Mesh] We use as input a two-dimensional rectangular domain of fixed size 300$\times$150 tessellated with triangles. We will vary the mesh spacing $h$, which impacts the number of elements in the domain.
\end{description}
...
Five fusion schemes were devised, based on the following criteria: (i) amount of data reuse, (ii) amount of redundant computation over the boundary region, (iii) memory footprint (the larger, the smaller the tile size to fit in cache). The fusion schemes are summarized in Table~\ref{table:seigen-fusion-schemes} and illustrated in Figure~\ref{fig:tiling:fusion-schemes}. The full specification, along with the seed tile size for each sub loop chain, is available at~\cite{seigen-code}.
...
\makebox[\textwidth][c]
{
\begin{tabulary}{1.0\columnwidth}{M{0.9cm} | M{2.2cm} | M{7.8cm} | M{1.0cm} N}
\hline
Fusion scheme & Number of sub loop chains & Criterion & {\em S-depth}  \\
\hlineB{4}
{\tt fs1} & 3 & Fuse only a few cells and facets loops & 2 & \\[5pt] \hline
{\tt fs2} & 8 & Several short loop chains & 2 & \\[5pt] \hline
{\tt fs3} & 6 & {\tt fs2} and include all solver loops & 2 & \\[5pt] \hline
{\tt fs4} & 3 & More aggressive than {\tt fs3} & 3 & \\[5pt] \hline
{\tt fs5} & 2 & Two loop chains: one for {\tt velocity} and one for {\tt stress} & 4 & \\[5pt] \hline
\end{tabulary}
}
...
\begin{figure}[htpb]
\centering
\includegraphics[scale=0.87]{sparsetiling/figures/loop_chains.pdf}
\caption{The sub loop chains in the five fusion schemes. {\it Cells}, {\it int-facets}, and {\it ext-facets} represent respectively the cell, interior facet, and exterior facet assembly loops, while {\it mat-vec} represents a solver.}
\label{fig:tiling:fusion-schemes}
\end{figure}
...
\label{sec:tiling:seigen:timing}
...
\begin{itemize}
\item All experiments were executed with ``warm cache''; that is, with all kernels retrieved directly from the Firedrake's software cache of compiled kernels, so code generation and compilation times are not counted.
...
\item We motivate, in detail, through plots and the {\it loop chain analyzer} tool (introduced shortly), the performance of the various fusion schemes and optimizations attempted.
...
\item The performance improvements tend to increase with $q$. This is remarkable, but not surprising. The memory footprint of an iteration grows with $q$, hence all negative effects such as impaired hardware prefetching and TLB misses (properly documented in the next sections) caused by sparse tiling become less notable. This trend can be observed in all problem instances. 
\end{itemize}
...
We experimented with $2 \times 4 \times 2 = 16$ {\tt explosive$\_$source} instances. For each of these instances, we tested the original Seigen version and a set of tiled versions. When $q \in \lbrace 1, 2 \rbrace$, a set of tiled versions consists of $5 \times 4 \times 2 \times 2 \times 2 = 160$ instances (otherwise, with $q \in \lbrace 3, 4 \rbrace$, the instances are 120). In total, we executed $8 \times (160 + 1) + 8 \times (120 + 1) = 2256$ versions of Seigen on single node. 
...
\subsubsection{On the Inspection Overhead}
As explained in Section~\ref{sec:tiling:seigen:timing}, the inspection cost is included in OT. In this section, we quantify this overhead in a representative problem instance. In all other problem instances, the overhead was either significantly smaller or essentially identical (for reasons discussed below) to the one reported here.

We consider {\tt explosive$\_$source} on Erebus with $h=1.0$, $q=4$, and {\tt fs5}. With this configuration, the time step was $t=481 \cdot 10^{-6}$ (we recall from Section~\ref{sec:tiling:seigen:timing} that $t$ is a function of $h$). Given the fixed simulation duration $T=2.5$, in this test case 5198 time steps were performed. A time step took on average 1.15 seconds.

In each time step, twenty-five loops, fused as specified by {\tt fs5}, are executed. We know that in {\tt fs5} there are two sub loop chains, which respectively consist of thirteen and twelve loops. To inspect these two loop chains, 1.4 and 1.3 seconds were needed (average across the four MPI ranks, with negligible variance). Roughly 98$\%$ of the inspection time was due to the projection and tiling functions, while only 0.2$\%$ was spent on the tile initialization phase (see Section~\ref{sec:tiling:inspector}). These proportions are consistent across other fusion schemes and test cases.

After 200 time steps (less than 4$\%$ of the total) the inspection overhead was already close to 1$\%$. Consequently, the inspection cost, in this test case, was eventually negligible. The inspection cost increases with the number of fused loops, which motivates the choice of {\tt fs5} for this analysis. 
...
Sparse tiling, and more generally time tiling, aims to turn the data reuse in a sequence of loops into data locality. In this chapter, three main problems have been addressed: the generalization of sparse tiling to arbitrary sequences of irregular loops through the loop chain abstraction, automation via DSLs, and effective support for shared- and distributed-memory parallelism. The major strength of this work lies in the fact that all algorithmic and technological contributions derive from an in-depth study of a realistic applications. The performance issues we found through Seigen would never have been exposed by a set of simplistic benchmarks, as often used in the literature.
...
\begin{description}
\item[Multi-node experiments] The more aggressive fusion schemes, which make use of extended halo regions, may be affected or take advantage from network communication. Other issues or potentials inherent in sparse tiling could be exposed. Multi-node experiments have highest priority and are now in progress.
\item[3D problems] Since many real-world simulations are based on 3D problems, it would be natural to extend the performance evaluation of Seigen in this direction. At the moment of writing, however, Seigen still does not include 3D test cases.
