...
The pseudo-code for the generalized sparse tiling inspector is showed in Algorithm~\ref{algo:st-inspector}. The output is a schedule that supports mixed distributed/shared memory parallelism. In the following, we describe the main steps of the algorithm.
...
\kwInput{The loop chain, $\mathbb{L} = [L_0,\ L_1,\ ...,\ L_{n-1}]$}
\kwInput{The tile size, $ts$}
...
\paragraph{Creation of tiles}
The algorithm starts with partitioning $S_{seed}^{core}$, the core region of $S_{seed}$, into $\lbrace P_0, P_1, ..., P_{m-1}\rbrace$, such that $P_i \cap P_j = \emptyset$ and $\cup_{i = 0}^{m-1} P_i = S_{core}$. The partitioning is sequential: $S_{seed}$ is ``chunked'' every $ts$ iterations, with $P_{m-1}$ that may be smaller than all other partitions. Similarly, we partition $S_{seed}^{bound}$ into $k$ subsets.

We then create $m+k+1$ tiles, $\mathbb{T} = \lbrace T_0, ..., T_{m+k} \rbrace$, one for each partition and one extra tile for $S_{seed}^{nexec}$. Each tile $T_i$ has four fields: a region (where possible values are core, boundary and non-exec); a color; for each loop $L$, a list of iterations $T_i^L$ (initially empty, to be populated by the inspection algorithm); for each loop, a list of ``local maps'', whose role will become clear next. This structuring is illustrated in Figure~\ref{fig:tile-structure}. At this point we have, for each $T_i \in \mathbb{T}$, $T_i^{L_{seed}} = P_i$. 

Each tile needs be assigned a color, which represents a scheduling priority. If shared memory parallelism is requested, color the tiles such that adjacent tiles are not given the same color. Otherwise, assign increasing colors to tiles (i.e., $T_i$ given color $i$). The boundary tiles are always given colors higher than that of core tiles; the non-exec tile has highest color. In essence, this is to provide the executor with sufficient information to schedule all core tiles first, then all boundary ones, and finally the non-exec tile. The adjacency relation is determined through the maps available through the descriptors of the loop chain.

\paragraph{Population of tiles}
Let $\phi$ be the data structure tracking projections for each space in the loop chain. Let $\sigma_{seed} : S_{seed} \rightarrow \mathbb{T}$ be the tiling function for $L_{seed}$, which maps iterations to tiles as determined at step (3). Then, for each loop $L_i$ in $[L_1, L_2, ..., L_{n-1}]$:
\begin{itemize}
...
\paragraph{Local maps}
Local maps are used during execution in place of the global maps provided by the loop chain, allowing sequential loops without the need for double indirections.
