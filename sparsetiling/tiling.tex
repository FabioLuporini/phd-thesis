...
MPI version & Open MPI 1.6.5  & Intel MPI 5.1 & Intel MPI 5.1 & \\[5pt] \hline
\end{tabulary}
}
...
The execution times that we report below derive from runs at maximum parallelism degree (i.e., 1 MPI process per physical core). In other types of runs, for instance sequential runs, the tiled versions tend to be outperformed by the original Seigen. This is a consequence of two factors: (i) the memory access latency is not affected if the bandwidth is shared by only a small set of processes; (ii) the memory access pattern is more irregular in the tiled versions, which severely impairs hardware prefetching. 
...
\caption{Comparison between the original and tiled versions of Seigen on Erebus, CX1-Haswell, and CX1-Ivy. 4 physical cores are used on Erebus, while 20 physical cores are used on both CX1-Haswell and CX1-Ivy.}
...
\begin{description}
\item[Fusion scheme] The six fusion schemes studied, {\tt fs}$\lbrace${\tt 1, 2, 3, 4, 5, 6}$\rbrace$, were summarized in Table~\ref{table:seigen-fusion-schemes}.
\item[Seed tile size] For each {\tt fs} and $q$, three or four tile sizes were tested. Four tile sizes were used with $q \in \lbrace 1, 2\rbrace$. The tile sizes were chosen to maximize the likelihood of a tile to fit in L2 or L3 cache. A less extensive experimentation with adverse tile sizes showed that: (i) a very small tile size causes dramatic slow-downs (up to 8$\times$ slower than the original versions); (ii) larger tile sizes cause proportionately greater slow-downs.
\item[Indirection maps] Two types of maps were tested: local and global. 
\item[Partitioning] Two partitioning strategies were tested: {\tt chunk} and {\tt metis}.
\item[Extended boundary region] Three fusion schemes out of six may benefit from an extended boundary region to minimize redundant computation.
\end{description}

Software prefetching and a small set of prefetch distances were also tested in a few instances of {\tt explosive$\_$source}, but no significant changes to the execution times were observed. It was decided, therefore, to exclude this parameter from the search space; this made it possible executing the whole test suite in slightly less than five days. 

We know that an {\tt explosive$\_$source} instance is uniquely determined by three parameters:
...
\item[Mesh] The meshes shown in Table~\ref{table:seigen-meshes} were used, with two meshes for each node architecture.
\end{description}

Overall, we experimented with $3 \times 4 \times 2 = 24$ {\tt explosive$\_$source} instances. For each of these instances, we tested the original Seigen version and a set of tiled versions. When $q \in \lbrace 1, 2 \rbrace$, a set of tiled version consists of $3 \times 4 \times 2 \times 2 \times 1 + 3 \times 4 \times 2 \times 2 \times 2 = 144$ instances (otherwise, with $q \in \lbrace 3, 4 \rbrace$, the instances are 108). In total, we executed $12 \times (144 + 1) + 12 \times (108 + 1) = 3048$ instances of Seigen. 

Given the large number of problem instances, the search space exploration is organized as follows. We will show one plot for each of the 24 {\tt explosive$\_$source} instances. Each plot illustrates the search space exploration for a given node architecture, mesh and $q$. The execution times of the original version and of a subset of tiled versions are shown. The impact of the indirection maps and the extended boundary region optimization on a tiled version (i.e., a point in a plot) will only be discussed qualitatively; a point, therefore, represents the best execution time out of these two parameters. It is quite straightforward to draw general conclusions about the indirection maps and extended boundary region optimization, so it was decided to remove them from the plots for increased readability. The raw execution times are available at~\cite{seigen-times-raw-values}.
