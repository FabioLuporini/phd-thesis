...
In this section, we apply sparse tiling to a real-world computation written in Firedrake, Seigen. The performance analysis is quite vast and it can be considered an actual methodological contribution of this thesis.
...
\item The runs on CX1-Hawell and CX1-Ivy were performed with exclusive access to the machines. Processes are pinned to cores.
\end{itemize}
...
\makebox[\textwidth][c]
{
\begin{tabulary}{1.2\textwidth}{M{2.2cm} | M{3.1cm} | M{3.1cm} | M{3.1cm} N}
\hline
System & Erebus & CX1-Haswell & CX1-Ivy \\
\hline \hline 
Node & 1x4-core Intel I7-2600 3.4GHz & 2x20-core Intel Xeon E5-2650 2.30GHz & 2x20-core Intel Xeon E5-2660 2.20GHz & \\[15pt] \hline
Memory & 16GB & 64GB & 64GB & \\[5pt] \hline
Cache hierarchy & L1=32KB, L2=256KB, L3=8MB (shared) & & & \\[15pt] \hline
Compilers & Intel {\tt icc} 16.0.2 & Intel {\tt icc} 15.1 & Intel {\tt icc} 15.1 & \\[5pt] \hline
Compiler flags & {\tt -O3 -xHost -ip} & {\tt -O3 -xHost -ip} & {\tt -O3 -xHost -ip} & \\[5pt] \hline
\end{tabulary}
}
...
$n$ & $h$ & $d$  \\
\hline \hline
\multirow{4}{*}{\shortstack{4 \\ (Erebus)}} & \multirow{2}{*}{1.0} & \multirow{2}{*}{...} \\
& & \\
& \multirow{2}{*}{1.2} & \multirow{2}{*}{...}  \\
& & \\
\hline \hline
\multirow{4}{*}{\shortstack{20 \\ (CX1-Hawell, \\ CX1-Ivy)}} & \multirow{2}{*}{0.6} & \multirow{2}{*}{...} \\
& & \\
& \multirow{2}{*}{0.8} & \multirow{2}{*}{...}\\
& & \\
\hline
\end{tabulary}
...
\makebox[\textwidth][c]
{
\begin{tabulary}{1.0\columnwidth}{M{1.2cm} | M{2.0cm} | M{7.2cm} | M{1.7cm} N}
\hline
Fusion scheme & $\#$(sub-)loop chains & Criterion & {\em S-depth}  \\
\hline \hline
...
{\tt fs6} & 1 & The extreme case: fuse all of the 25 loops & 8 & \\[5pt]
\hline
\end{tabulary}
}
...
\begin{description}
\item[Global and local maps] Algorithm~\ref{algo:st-inspector} computes so called local maps to avoid double indirections in the executor. We regard this as a {\it potential optimization}. Although no data reuse is available for the local maps (each fused loop has its own local maps), the benefits from improved hardware prefetching and memory latency could be dominant. We will experiment with both global (i.e., original, as constructed by Firedrake) and local (i.e., computed by SLOPE) maps. The runs using local maps will be represented with the suffix {\tt lm}.

\item[Tile shape] The seed loop partitioning sets the tile shape. The simplest way of creating tiles consists of ``chunking'' the seed iteration space every $ts$ iterations, with $ts$ being the ``initial'' tile size, as explained in Section~\ref{sec:tiling:inspector}. The {\tt chunk} partitioning inherits the original mesh numbering. In Firedrake, and therefore in Seigen, meshes are renumbered during initialization applying the reverse Cuthill–McKee (RCM) algorithm. Using {\tt chunk} partitioning on top of an RCM-renumbered mesh has the effect of producing thin, rectangular tiles. This affects tile expansion as a large proportion of elements lie on the tile border. There are potential solutions to this problem. The most promising one is using a Hilbert curve, rather than RCM, to renumber the mesh. This would lead to ``blobby'' tiles when applying {\tt chunk} partitioning. Unfortunately, Hilbert curves are at the moment not supported in Firedrake. Another alternative consists of partitioning the seed iteration space with a library like METIS before applying RCM. As we shall see, despite improving the tile shape, this strategy is not side effects free. We refer to it as {\tt metis} partitioning.

\item[Software prefetching] There may be more than one regular memory access pattern amenable to hardware prefetching (e.g., accesses to the indirection maps; direct accesses to data values; indirect accesses to data values if the mesh has a good numbering). Sparse tiling, unfortunately, may impair hardware prefetching because of two reasons: (i) the virtual addresses streams are considerably shorter; (ii) tile expansion introduces irregularities in these streams. Software prefetching can be used together with hardware prefetching to minimize memory stalls. PyOP2 and SLOPE have been extended to emit intrinsics instructions for prefetching the iteration $i$'s maps and data values while executing iteration $i-1$. The runs using software prefetching will be represented with the suffix {\tt sw-pre}.

\item[Extended boundary region] The special non-exec tile $T_{ne}$ can reduce the amount of redundant computation in long loop chains by expanding over the boundary tiles. To create $T_{ne}$ there are two options: either an extra level of off-process data, marked as ``non-exec'', is communicated or the inspection has additional logic. The current implementation only supports the first option as deriving $T_{ne}$ in the inspection would be not only complex, but also particularly costly. The fusion schemes {\tt fs1}, {\tt fs2}, and {\tt fs3} do not benefit from the presence of $T_{ne}$, so by default no extra off-process data is communicated. The other fusion schemes may instead benefit from this optimization, which in the following will be referred to as {\tt extra-halo}.
\end{description}


\subsection{Results}
\label{sec:tiling:seigen-results}
In this section, we report on hundreds of experiments aimed at quantifying the performance improvements obtainable in Seigen by applying sparse tiling. We use the {\tt explosive$\_$source} test case and we explore a large set of parameters, which includes polynomial order, mesh, architecture, fusion scheme, and others. A generalization of the study -- in particular, of the factors limiting sparse tiling -- to different programs is also provided. In the following, a generic instance of {\tt explosive$\_$source} making use of sparse tiling is referred to as a ``tiled version'', otherwise the term ``original version'' is used.

\subsubsection{Summary of the Attained Speed-ups}
The support for shared-memory parallelism is discontinued in Firedrake, so only distributed-memory tiled versions were tested. The execution times reported derive from runs at maximum parallelism degree, that is, with 1 MPI process on each physical core. Other types of runs, for instance sequential runs, are less interesting as the tiled versions are almost always outperformed by the original Seigen. This is a results of two main factors: (i) when the bandwidth is shared by a small set of processes, the memory access latency is relatively low; (ii) the memory access pattern is more irregular in the tiled versions, which severely impairs hardware prefetching. 

We here start with showing the maximum speed-ups achieved by the tiled versions on Erebus, CX1-Haswell, and CX1-Ivy. The maximum speed-up was determined by picking the best execution time out of a set of tiled versions, for each node, polynomial order $q$, and mesh. The search for the best tiled version is described in the next section. The results are summarized in Table~\ref{table:seigen-speedups}.
...
\caption{Comparison between the original and tiled versions of Seigen on three different platforms. All physical cores are used (1 MPI process per core, no hyperthreading).}
\label{table:seigen-speedups}
\end{table}

Overall, the performance improvements tend to be greater than 1.10$\times$, and sometimes even exceed 1.20$\times$. In light of the limiting factors discussed in the next sections, this is extremely promising. Indeed, while some of these limiting factors are inherent in the sparse tiling execution model, others are simply due to the current implementation of Seigen/Firedrake, so we envisage ample space for larger improvements. 

%Moreover, scaling up to architectures with smaller available bandwidth per core


\subsubsection{In-depth Search Space Exploration}
...
\item[Constrained tile size] The memory footprint of a tile grows quite rapidly with the number of fused loops. In particular, the matrices accessed in the {\tt velocity} and {\tt stress} solver loops have a considerable impact. The larger the memory footprint, the smaller the tile size to fit in cache. Allocating small tiles has multiple implications. The proportion of iterations lying on the border is often bigger than in the big tiles case. Consequently, tile expansion (see Section~\ref{sec:tiling:examples}) may be more aggressive, thus affecting data locality. Using small tiles also impairs hardware prefetching, since the virtual addresses streams are interrupted with higher frequency. Moreover, more tiles are needed to cover an iteration space. In the case of shared-memory parallelism, the larger the number of tiles, the higher the probability of detecting a coloring conflict, which results in higher inspection costs.

\item[Limited data reuse in small loop chains] With a large memory footprint, a fusion scheme with multiple, shorter loop chains (e.g., 2-3 loops each) could appear, at first glance, more promising: the data reuse in consecutive loops can be turned into data locality resorting to a small {\em S-depth} and a relatively big tile, hence mitigating most of the aforementioned issues. However, other problems arise. By limiting the extent of a loop chain, it may not be possible to fuse some of the {\tt velocity} and {\tt stress} solver loops. These loops are memory-bound, share a significant volume of data, and have a considerable impact on the execution time, so sparse tiling could provide substantial benefits. A second issue is inherent to the fact that data reuse is much smaller in short loop chains. Consider two loops $L_x$ and $L_y$, respectively over cells and interior facets. For a given tile, during the execution of iterations from $L_x$, new fields and indirection maps are loaded from memory into cache. When executing iterations from $L_y$, new indirection maps need be fetched from memory (since $L_y$ has a different iteration space than $L_x$, hence different maps). Locality on the shared data values is then exploitable, but it is evident that the ratio data reuse over total data movement could, in some cases, really be small.
