...
In the case of distributed-memory parallelism, the mesh is partitioned and distributed to multiple processes. As shown in Listing~\ref{code:tiling-struct}, neighboring processes may exchange (MPI) messages before executing a loop $L_j$. A message includes all dataset values required by $L_j$ modified by any $L_k$, with $L_k \prec L_j$. In the running example, $L_0$ writes to vertices, so a subset of values associated with border vertices must be communicated prior to the execution of $L_1$. To apply sparse tiling, the idea is to push all communications at the beginning of the loop chain: as we shall see, this increases the amount of data to be communicated, but also reduces  synchronization (only 1 synchronization between each pair of neighboring processes per loop chain execution).

From Section~\ref{sec:tiling:lc-unstruct}, we know that a set in a loop chain is logically split into three regions, \textit{core}, \textit{boundary}, and \textit{non-exec}. The boundary region includes all iterations that cannot be executed until the required MPI communications have terminated. Taking this into consideration, we modify the procedure described for shared-memory parallelism as follows:
\begin{enumerate}
\item We take the core region of the seed loop $L_0$ and partition it into tiles. Unless aiming for a mixed distributed/shared-memory scheme, there is no need to assign identical colors to unconnected tiles, as processes execute their own tiles sequentially. We can assign colors increasingly, with $T_i$ given color $i$. As long as tiles with contiguous ID are also physically contiguous in the mesh, this assignment retains spatial locality when ``jumping'' from executing $T_i$ to $T_{i+1}$.
\item The same process is applied to the boundary region. Thus, by construction, a situation in which a tile includes iterations from both regions is prevented. Further, all tiles within the boundary region are assigned colors higher than those used in the core region. This constrains the execution order: no boundary tiles will be executed until all core tiles are computed.
...
The schedule produced by the inspector is subsequently used by the executor. On each process, the executor starts with triggering the MPI communications required to the computation of the boundary tiles. All core tiles are then computed, since no data from the boundary region is necessary. Hence, computation is overlapped with communication. As all core tiles are computed and the MPI communications performed, the boundary tiles can finally be computed.

In Figure~\ref{fig:st-mpi-init}, a mesh is distributed to two processes and a possible tiling of $L_0$ illustrated. It is worth observing that: (i) the tiles in the boundary region will expand over the core tiles as $L_1$ and $L_2$ are scheduled; (ii) the size of the boundary region is such that no synchronizations amongst neighboring processes are required while executing the core tiles first, and the boundary tiles after. Intuitively, this is because enough information is provided before approaching the execution of the fused sequence of loops. Roughly speaking, if a loop chain consists of $n$ loops, then the boundary region may need up to $n-1$ extra layers of iterations. 
...
Our hypothesis for this inspector/executor scheme is that the increase in data locality will outweigh the overhead induced by the redundant computation and by the bigger volume of data exchanged. This is motivated by several observations: (i) the loops being memory-bound;  (ii) the core region being much larger than the boundary region; (iii) the amount of redundant computation being minimized through the special tile $T_{ne}$, will progressively expands over the boundary region and thus avoids unnecessary calculations.
